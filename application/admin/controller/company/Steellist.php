...
class Steellist extends Backend
{
    ...
    public function index()
    {
        ...
        if ($this->request->isAjax())
        {
            ...
            if(!$total){
                ...
                if($steelArray){
                    //$time_temp = strtotime($time);
                    $steelContents = $steelArray['contents'];
                    foreach ($steelContents as $key => $value) {
                        $steelContents[$key]['date'] = $time;
                    }

                    $result = $this->model->saveAll($steelContents);
            ...
            foreach ($list as $row) {
                $row->visible(['date','breed','category','material','place','price','raise','rowId','spec','unit','note']);
            }
    ...
    private function refresh($time = null){
